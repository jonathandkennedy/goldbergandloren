...
| `maximize-settlement.html` | "Do I even need a lawyer?" |
...
## "Maximize Your Compensation" → "Maximize Your Settlement" (2026-09-22)
The page's H1 is now **Maximize your *settlement*** (Spanish: *Maximice su
acuerdo*), and its URL moved to match: `maximize-settlement.html`.

- **The old URL still works.** `maximize-compensation.html` is now a tiny
  `noindex` redirect to the new page. It carries the query string and hash
  across, so `?geo=` and `gclid` survive and CallRail keeps attributing. It is
  generated by the same script (`RENAMED` in `generate-support-pages.mjs`).
- **Update the sitelink in Google Ads** to the new URL and the new text,
  **Maximize Your Settlement** (24 chars). `SITELINKS.md` is updated. The
  redirect covers the gap, but a direct URL saves visitors a hop.
